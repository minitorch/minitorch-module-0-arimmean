...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert 0.0 <= s <= 1.0
    s_neg = sigmoid(-a)
    assert_close(1.0 - s, s_neg)
    assert_close(sigmoid(0.0), 0.5)

    eps = 1e-6
    s_minus = sigmoid(a - eps)
    s_plus = sigmoid(a + eps)

    if 1e-6 < s_minus < 1 - 1e-6 and 1e-6 < s_plus < 1 - 1e-6:
...
def test_other(x: float) -> None:
    ...
    """Test that exp is reverse function of log and visa versa."""
    assert_close(log(exp(x)), x)
    # assert_close(exp(log(abs(x)), abs(x)))
